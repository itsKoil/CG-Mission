...
waitUntil {life_dog_order != 5};
		...
		if(_side countSide _nearestunits > 0) then
			...
			if ((dog distance getpos (_nearestunitofside select 0)) < 2 && (animationState (_nearestunitofside select 0)) != "BasicDriverOutDying") then 
			{
				...
				[[_attackperson,"BasicDriverOutDying"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
				...
				[[_attackperson,"AmovPpneMstpSnonWnonDnon_healed"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
			} else 
			{
		...
		if (life_dog_order == 4 || (!alive dog) || (!alive player)) exitwith {
			sleep 5;
			deletevehicle dog;
			player removeAction _acts1;
			player removeAction _acts2;
