...
player setVariable["defect_vehicle",false,true];
...
[] spawn {
	...
	while {true} do {
		...
		if(_server != "CriticalGaming TeamSpeak Server" or _channel != "TaskForceRadio") then {
			
			cutText["Wrong TS Channel.","BLACK",0,true];
			waitUntil {([] call TFAR_fnc_getTeamSpeakServerName) == "CriticalGaming TeamSpeak Server" and ([] call TFAR_fnc_getTeamSpeakChannelName) == "TaskForceRadio"};
			cutText["","PLAIN"];
